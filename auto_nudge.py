...
async def _auto_nudge_async(duration, session=None):
    """
    ...
    try:
        ...
        if status == "playing":
            ...
            await play()
            await asyncio.sleep(0.5)  # Give the media player a moment to update its state
            status = await get_status()
            if status == "playing":
...
def trigger_auto_nudge(duration=0.3, session=None):
    """Run the auto-nudge coroutine on the app's shared background event loop
    instead of spinning up a new thread + event loop for every call."""
    run_async(_auto_nudge_async(duration, session))